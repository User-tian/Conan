...
class TechBinarizer(SingingBinarizer):
    ...
    def process_item(cls, item, binarization_args):
        item_name = item['item_name']
        wav_fn = item['wav_fn']
        if 'other' in wav_fn.lower():
            return None
        wav, mel = cls.process_audio(wav_fn, item, binarization_args)
        mel=item['mel']
        wav=item['wav']
        if type(item['hubert']) == str:
            content = [float(x) for x in item['hubert'].split()]
        elif type(item['hubert']) == list:
            content = [float(x) for x in item['hubert']]
        ...
        # try:
        f0 = np.load(wav_fn.replace(".wav", "_f0.npy"))[:mel.shape[0]]
        if hparams['hop_size']==160:
            content = [elem for elem in content for _ in range(2)]
        # print(f'content:{len(content)},mel:{len(mel)},f0:{len(f0)}')
        ...
        item['len']=min_length
        # print(f'f0_length: {f0.shape}, mel_length: {mel.shape},wav_length: {wav.shape}')
        # except:
        #     # parselmouth
