...
class SqueezeformerLayers(nn.Module):
    """
    Squeezeformer Layers first processes the input with a convolution subsampling layer and then
    with a number of squeezeformer blocks.
    ...
    def __init__(
        ...
        super(SqueezeformerLayers, self).__init__()
        self.num_layers = num_layers
        self.reduce_layer_index = reduce_layer_index
        self.recover_layer_index = recover_layer_index
        self.time_reduction_layer = TimeReductionLayer()
        ...
        self.time_recover_layer = nn.Linear(encoder_dim, encoder_dim)
        self.recover_tensor = None
        if reduce_layer_index < 0:
            recover_layer_index = num_layers
    ...
    def forward(self, inputs: Tensor, input_lengths: Tensor = None) -> Tuple[Tensor, Tensor]:
        ...
        #     output_lengths = [inputs.shape[1]] * inputs.shape[0]
        #     output_lengths = torch.IntTensor(output_lengths).to(inputs.device)
        for idx, layer in enumerate(self.layers):
            if idx == self.reduce_layer_index:
                self.recover_tensor = outputs
                outputs, output_lengths = self.time_reduction_layer(outputs, output_lengths)
