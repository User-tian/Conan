...
from modules.TechDet.unet import Unet
from modules.TechDet.squeezeformer.encoder import SqueezeformerLayers
...
class TechExtractor(nn.Module):
    def __init__(self, hparams):
        ...
        # squeezeformer
        elif hparams.get('bkb_net', 'conv') == 'squeezeformer':
            mid_net = SqueezeformerLayers(hidden_size, num_layers=hparams.get('bkb_layers', 2), 
                        num_attention_heads=4, feed_forward_dropout_p=self.dropout,
                        attention_dropout_p=self.dropout, conv_dropout_p=self.dropout, conv_kernel_size=hparams.get('squeezeformer_kernel', 9))
        ...
        self.post = ConvBlocks(hidden_size, out_dims=hidden_size, dilations=None, kernel_size=3,
                               layers_in_block=1, c_multiple=1, dropout=self.dropout, num_layers=1,
                               post_net_kernel=3, act_type='swish')
        self.tech_out = nn.Linear(hidden_size, hparams.get('binary_tech_num', 5))
        self.mf_tech_out = nn.Linear(hidden_size, 3)
        self.sw_tech_out = nn.Linear(hidden_size, 3)
    ...
    def forward(self, mel=None, ph_bd=None, pitch=None, uv=None, variance=None, non_padding=None, train=True):
        ...
        )  # [B, T] -> [B, ph_length] count the phone frames of each phone (with padding excluded)
        ...
        mf_tech_logits = self.mf_tech_out(tech_logits)
        mf_tech_pred = torch.softmax(mf_tech_logits, dim=-1)
        mf_tech_pred = torch.argmax(mf_tech_pred, dim=-1)
        ret['mf_tech_logits'] = mf_tech_logits
        ret['mf_tech_pred'] = mf_tech_pred

        sw_tech_logits = self.sw_tech_out(tech_logits)
        sw_tech_pred = torch.softmax(sw_tech_logits, dim=-1)
        sw_tech_pred = torch.argmax(sw_tech_pred, dim=-1)
        ret['sw_tech_logits'] = sw_tech_logits
        ret['sw_tech_pred'] = sw_tech_pred
        
        tech_logits = self.tech_out(tech_logits) / self.tech_temperature
        ret['tech_logits'] = tech_logits
        tech_pred = torch.sigmoid(tech_logits) 
    ...
    def reset_parameters(self):
        ...
        nn.init.kaiming_normal_(self.mf_tech_out.weight, mode='fan_in')
        nn.init.kaiming_normal_(self.sw_tech_out.weight, mode='fan_in')
        nn.init.constant_(self.multihead_dot_attn.bias, 0.0)
        nn.init.constant_(self.mel_proj.bias, 0.0)
        nn.init.constant_(self.tech_out.bias, 0.0)
        nn.init.constant_(self.mf_tech_out.bias, 0.0)
        nn.init.constant_(self.sw_tech_out.bias, 0.0)
