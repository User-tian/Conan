...
from modules.TechSinger.diff.gaussian_multinomial_diffusion import (
    GaussianMultinomialDiffusion,
    GaussianMultinomialDiffusionx0,
)
from modules.TechSinger.diff.net import DiffNet, F0DiffNet, OriDiffNet, CausalConv1d
...
from modules.TechSinger.prosody_util import ProsodyAligner, LocalStyleAdaptor
...
class RFSinger(FastSpeech):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if spk_embed is not None:
            ...
            # 如果没有提供 spk_embed，则从 target mel 频谱中提取
            if ref is None:
                raise ValueError(
                    "当 spk_embed 为 None 时，需要 target 张量来提取说话人嵌入。"
                )
            # encode_spk_embed 可能包含非因果操作（如全局编码器），但最终输出是 avg_pool 后的静态向量
            ret["style_embed"] = style_embed = self.encode_spk_embed(
                ref.transpose(1, 2)
            ).transpose(1, 2)

        # pitch 输入 = content 嵌入 + style 嵌入
        ...
        # --- 调用 forward_pitch 计算音高嵌入，并将 kwargs (包含 initial_noise) 传递下去 ---
        # f0 和 uv 可能是外部传入（训练时），或在 forward_pitch 内部生成（推理时）
        if infer:
            f0, uv = None, None
        # ret['f0_denorm_pred'] 会在 forward_pitch 内部被填充
        ...
        # --- 调用 forward_decoder 生成 Mel 输出 ---
        # 假设 forward_decoder 内部处理长度调节（从 token 到 frame）和掩码应用
        # 如果 forward_decoder 需要帧级别的掩码，需要确保它能正确获取或推断
        ret["mel_out"] = self.forward_decoder(
            decoder_inp, tgt_nonpadding, ret, infer=infer, **kwargs
        )  # 也传递 kwargs 以防解码器需要
        ret["tgt_nonpadding"] = tgt_nonpadding
        # === 将 ret 中由 ReflowF0 存储的 initial_noise_used (如果存在) 提升到顶层 ret ===
        # 这样测试函数就可以直接从 model() 返回的字典中获取它
        if "initial_noise_used" in ret:
            # 注意：这里是字典内部赋值，实际上 ret 已经是同一个字典，
            # 只需要确保 'initial_noise_used' 键存在于最终返回的 ret 中即可。
            # 可以加一句 ret['initial_noise_used'] = ret['initial_noise_used'] 来明确，但非必需。
            pass  # 确保键存在即可

        return ret  # 返回包含所有结果的字典
    ...
    def get_prosody(self, encoder_out, ref_mels, ret, infer=False, global_steps=0):
        ...
        if "ref_upsample" not in ret or ret["ref_upsample"] is None:
            B, T, _ = ref_mels.shape  # 取 batch 和帧长
    ...
    def forward_pitch(self, decoder_inp, f0, uv, ret, **kwargs):  # 添加 **kwargs
        ...
        if hparams["f0_gen"] == "diff":
            ...
            raise ValueError(f"未知的 f0_gen 类型: {hparams['f0_gen']}")

        # --- 使用从 add_x_pitch 返回的 f0_out, uv_out ---
        # f0_out 可能是 log F0 或其他形式，denorm_f0 应该能处理
        f0_denorm = denorm_f0(f0_out, uv_out)  # 使用返回的 f0 和 uv 进行反归一化
        pitch = f0_to_coarse(f0_denorm)  # 转换为音高类别
        ret["f0_denorm_pred"] = f0_denorm  # 在 ret 中存储最终预测的反归一化 F0
        pitch_embed = self.pitch_embed(pitch)  # 计算音高嵌入
        return pitch_embed  # 返回音高嵌入
    ...
    def add_orig_pitch(self, decoder_inp, f0, uv, ret, encoder_out=None, **kwargs):
        ...
        if infer:
            ...
            if "content" in ret:
                content_padding_mask = ret["content"] == self.hparams['silent_token']  # 假设 -1 是 padding index
                if content_padding_mask.shape == uv.shape:
                    uv[content_padding_mask] = 1  # padding 区域强制设为无声
            uv = uv
            f0 = uv_pred[:, :, 1]
            ret["fdiff"] = 0.0
        else:
    ...
    def add_flow_pitch(self, decoder_inp, f0, uv, ret, encoder_out=None, **kwargs):
        ...
        # 定义 F0 归一化和反归一化函数 (minmax_norm, minmax_denorm)
        def minmax_norm(x, uv=None):
            x_min = 6
            x_max = 10
            # if torch.any(x > x_max): # 检查是否有超出范围的值 (可能只在训练时检查)
            #     # print(f"Warning: F0 value > {x_max} found during normalization.")
            #     pass # 或者可以选择 clip
            normed_x = (x - x_min) / (x_max - x_min) * 2 - 1
            if uv is not None:
                normed_x[uv > 0] = 0  # 无声区域设为 0
            return normed_x
        ...
        def minmax_denorm(x, uv=None):
            ...
            denormed_x = (x + 1) / 2 * (x_max - x_min) + x_min
            if uv is not None:
                denormed_x[uv > 0] = 0  # 无声区域设为 0
            return denormed_x

        # --- 从 kwargs 中提取 initial_noise ---
        initial_noise = kwargs.get("initial_noise", None)

        # 判断是推理还是训练模式
        if f0 is None:  # 如果没有提供 f0，则认为是推理模式
            infer = True
            if uv is None:  # 如果 uv 也未提供，需要从预测器获取
                # 确保 uv_predictor 已初始化
                if not hasattr(self, "uv_predictor"):
                    raise AttributeError("uv_predictor is not defined in the model.")
                #  uv_pred = self.uv_predictor(decoder_inp) # 预测 UV, 形状 [B, T_tok, 2]
                uv = uv_pred[:, :, 0] > 0  # 取第一个维度作为 UV 标志 (True 表示无声)
                # (可选) 应用 content padding 掩码到 UV 上
                if "content" in ret:
                    content_padding_mask = (
                        ret["content"] == self.hparams['silent_token']
                    )  # 假设 -1 是 padding index
                    if content_padding_mask.shape == uv.shape:
                        uv[content_padding_mask] = 1  # padding 区域强制设为无声
                    else:
                        print(
                            f"警告: content 掩码形状 {content_padding_mask.shape} 与 uv 形状 {uv.shape} 不匹配，无法应用。"
                        )
                else:
                    print(
                        "警告: ret 中缺少 'content'，无法将 content padding 应用到 UV。"
                    )

            # --- 调用 self.f0_gen (ReflowF0 实例) 进行 F0 预测，并传递 initial_noise ---
            # 输入 cond 需要是 [B, C, T] 形状
            # decoder_inp 是 [B, T, C]，需要 transpose
            # f0_gen 的输出应该是归一化后的 F0，形状 [B, T]
            ...
            ret["pflow"] = 0.0  # 推理时不计算 flow loss
            uv_out = uv  # 返回用于反归一化的 uv
        else:  # 如果提供了 f0，则认为是训练模式
            infer = False
            # 计算 nonpadding (发声区域掩码)
            nonpadding = (uv == 0).float()
            # 使用提供的 f0, uv 进行归一化
            norm_f0 = minmax_norm(f0, uv)
            # 调用 f0_gen 计算 flow loss，训练时通常不传入 initial_noise
            # f0_gen 训练时输入 norm_f0 需要是 [B, 1, 1, T] 或 [B, 1, D, T]
            # add_flow_pitch 接收的 f0 是 [B, T], norm_f0 也是 [B, T]
            # 需要在调用 f0_gen 前调整形状
            if norm_f0.ndim == 2:
                norm_f0_unsqueezed = norm_f0.unsqueeze(1).unsqueeze(1)  # -> [B, 1, 1, T]
            else:  # 如果已经是 [B, T, 1] 等其他形状，需要相应调整
                raise ValueError(f"训练时 norm_f0 形状未预期: {norm_f0.shape}")
            # nonpadding 需要是 [B, 1, T]
            ...
            f0_out = f0  # 训练时返回原始 f0
            uv_out = uv  # 训练时返回原始 uv

        return f0_out, uv_out  # 返回计算或原始的 f0 和 uv
